...
export const rejectConnection = async (req,res) => {
    try {
        ...
        return res.status(200).json({message:`connection rejected error ${error}`})
    }
}
...
export const getConnectionStatus = async (req,res) => {
    try {
        ...
        // if no connection or pending re found
        return res.json({status:"Connect"})
    } catch (error) {
        return res.status(200).json({message:`get connection error ${error}`})
    }
}
